...
class PlatlangLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (words(("niks", "woar", "neetwoar"), prefix=r"\b", suffix=r"\b"), Keyword.Constant),
            (
                words(("nommer", "nómmer", "teks", "portefeuil"), prefix=r"\b", suffix=r"\b"),
                Keyword.Type,
            ),
            (words(("aafdrokke", "aafdrökke", "invuier"), prefix=r"\b", suffix=r"\b"), Name.Builtin),
            (r"==|!=|<=|>=|[+\-*/%<>=!]", Operator),
            (r"[(){}\[\]:,.]", Punctuation),
            (r"[^\W\d]\w*", Name),
        ],
